...
//       am.getDBTransaction().
    ...
    public void saveAll(ActionEvent actionEvent) {
        ...
        try {
            ...
            am.getBomBpoLinesAllVO1().executeQuery();
            totalOrderQty.setValue(getTotalOrderQty(am.getBomBpoLinesAllVO1()));
            
            AdfFacesContext.getCurrentInstance().addPartialTarget(totalOrderQty);
    ...
    public void callUpdateBPO(ActionEvent actionEvent) {
        try {
            ...
            String pocId = getCurrentPocId(am.getCustMnjOntBomHeaderView1());
            System.out.println("POC_ID: " + pocId);
            
            callDeleteFromMNJ_BOM_BPO_LINES_ALL(pocId, am.getBomBpoLinesAllVO1());
            
            callInsertInMNJ_BOM_BPO_LINES_ALL(pocId);
            
            am.getDBTransaction().commit();
            
            System.out.println("Commit excecuted....");
    ...
    public int getTotalOrderQty(ViewObject vo){
        Row[] rowArray = vo.getAllRowsInRange();
        System.out.println("Total rows: " + rowArray.length);
        int sum = 0;
        for (Row eachRow : rowArray){
             int everyRowOrderedQty = Integer.parseInt(eachRow.getAttribute("OrderQty").toString());
             sum = sum + everyRowOrderedQty;
    ...
    public String getCurrentPocId(ViewObject vo){
        String pocId = vo.getCurrentRow().getAttribute("PocId").toString();
        return pocId;
    }
    
    
    public void callDeleteFromMNJ_BOM_BPO_LINES_ALL(String pocId, ViewObject voLine) throws SQLException {
        System.out.println("Enter in delete from mnj_bom_bpo_lines_all....");
        String sql = "delete from MNJ_BOM_BPO_LINES_ALL where POC_ID = ? ";
        PreparedStatement preparedStatement = 
            am.getDBTransaction().createPreparedStatement(sql, DBTransaction.DEFAULT);
        
        preparedStatement.setString(1, pocId);
        System.out.println("Total deleted rows in mnj_bom_bpo_lines_all: " + preparedStatement.executeUpdate());
        preparedStatement.execute();
        preparedStatement.close();
        
        voLine.executeQuery();
        System.out.println("vo line executed...");
    ...
    public void callInsertInMNJ_BOM_BPO_LINES_ALL(String pocId) throws SQLException {
    ...
    public void insertIntoMNJ_BOM_BPO_LINES_ALL(String bpoId, String bpoNo, String orderQty, ViewObject voBomLine, String pocId){
        
        Row newRow = voBomLine.createRow();
        newRow.setAttribute("SaleOrderId", bpoId);
        newRow.setAttribute("OrderQty", orderQty);
        newRow.setAttribute("BpoNo", bpoNo);
        newRow.setAttribute("PocId", pocId);
        voBomLine.insertRow(newRow);
        AdfFacesContext.getCurrentInstance().addPartialTarget(assignBpo_BindTB);
    }

    public void setTotalOrderQty(RichOutputText totalOrderQty) {
        this.totalOrderQty = totalOrderQty;
    }

    public RichOutputText getTotalOrderQty() {
        return totalOrderQty;
    }

    public String setTotalOrderQty() {
        try {
            am.getBomBpoLinesAllVO1().executeQuery();
